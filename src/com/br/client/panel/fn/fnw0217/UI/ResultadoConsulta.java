package com.br.client.panel.fn.fnw0217.UI;
...
import com.br.client.panel.fn.fnw0217.model.OptionView;
...
public class ResultadoConsulta extends HLayout{
	...
	public ResultadoConsulta(boolean callCentroCusto){
		...
		grupos.put("04", Tradutor.i18n.formGrupo04());
...
//		System.out.println(sql);
	...
	public String getSelect( boolean selectDetail, String mesRef, String conta, String sequencia, boolean centroCusto){
		...
		Date dataReferencia 		= this.getFiltroConsulta().getFieldMesReferencia().getField().getValueAsDate();
		// Date dataReferenciaFim		= this.getFiltroConsulta().getFieldMesReferenciaFinal().getField().getValueAsDate();

		// pega o ultimo dia do mes.
        // dataReferencia				= new Date(dataReferencia.getYear(), dataReferencia.getMonth()+1, 0);

		Date dataReferenciaAtrasos 	= this.getFiltroConsulta().getFieldMesReferencia().getField().getValueAsDate();
		dataReferenciaAtrasos		= new Date(dataReferencia.getYear(), dataReferencia.getMonth(), 1);

		
		boolean detalhaMes			= HowMGWTUtilities.getBoolean( this.getFiltroConsulta().getPropertyDetalhaMes().getHowMValue() );
		Date detalhaDataInicio		= this.getFiltroConsulta().getFieldDetalhaDataInicio().getField().getValueAsDate();
		Date detalhaDataFim			= this.getFiltroConsulta().getFieldDetalhaDataFim().getField().getValueAsDate();
		
		String tpPlanoConta = this.getFiltroConsulta().getPropertyTpPlanoConta().getHowMValue();
		...
		if( selectDetail ){
			...
			if( centroCusto ){
				...
				sqlFinal += "order by \n";
				sqlFinal += "	sequencia,\n";	
				sqlFinal += "	CONCAT( SUBSTRING( mesRef, 4, 4 ), SUBSTRING( mesRef, 1, 2 ) ),\n";
				sqlFinal += "	tipoConta,\n";
				sqlFinal += "	conta\n";
			}		
		}
		...
		for ( Object obj : entry ){
			...
			if( this.callCentroCusto ){
				...
				if( nomeMesAno == null || HowMGWTUtilities.getInteger(grupo) == HowMGWTUtilities.getInteger(EPlanoContas.grupoAtrasos)){
					...
					mes = HowMGWTUtilities.strZero(dataRef.getMonth()+1, 2)+":"+(dataRef.getYear()+1900);
				}
				else{
					nomeMesAno += " / "+mes.substring(3); // Imprime o rótulo para quando é pago ou a vencer.
				}
			}
			...
			if ( HowMGWTUtilities.strZero(HowMGWTUtilities.getInteger(EPlanoContas.grupoAtrasos), 2).equals(grupo) ){
				...
				campo.setAttribute("grupoTitle", Tradutor.i18n.formGrupo04() );	
			}
...
//					}
		...
		if( this.callCentroCusto ){			
			formBean.setTitle("Resultado Financeiro");
			formBean.setSubTitle("Centro de Custo");
			formBean.setModulo("FNW0221");
		}
		else{			
			formBean.setTitle("Resultado Financeiro");
			formBean.setSubTitle("Mes");
			formBean.setModulo("FNW0217");
		}
