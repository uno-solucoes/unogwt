...
public class EPlanoContas extends TreeNode{ 
	...
	public static final String getSelectTitulosOK(	
			...
			Date periodoFim,
			Date mesReferencia,
			// Date mesReferenciaFim,
			Date dataReferenciaAtrasos,
			boolean listReceita,
		...
		PlanoContasModel daoCP = new PlanoContasModelContasPagar(periodoInicio, periodoFim, mesReferencia, dataReferenciaAtrasos, detalhaMes, detalhaDataInicio, detalhaDataFim  , showCentroCusto , ccustoSubstring3);
		PlanoContasModel daoCR = new PlanoContasModel(periodoInicio, periodoFim, mesReferencia, dataReferenciaAtrasos, detalhaMes, detalhaDataInicio, detalhaDataFim  , showCentroCusto, ccustoSubstring3);
		...
		daoCP.setTpPlanoConta(tpPlanoConta);
		daoCR.setTpPlanoConta(tpPlanoConta);
		...
		if( listReceita ){
			// Pega dados MesAtual Contas Receber e Contas a Pagar
			select += daoCR.getSelectTitulosAVencerMesAtual();		// Alterar aqui - BEMARCO
			select += "\n";
			select += "UNION ALL";
			select += "\n";		
			// Pega dados outros meses pagos Contas Receber e Contas a Pagar
			select += daoCR.getSelectOutrosMesesPago();
			if ( optionView.isShowPrevisoes() ){
				select += "\n";
				select += "UNION ALL";
				select += "\n";
				// Pega dados titulos a vencer Contas Receber e Contas a Pagar
				select += daoCR.getSelectTitulosAVencer();
			}
			
			
			// Verificação problema de batimento entre o relatório : FNW0102 e o Resultado Financeiro.
			// OC: 73211 - Aberta pelo Marcio - Estou 100% Stressado com a Tudo.
			String selectAtrasos = daoCR.getSelectAtrasos();		// Alterar aqui - BEMARCO
			if ( optionView.isShowAtrasos() && ! HowMGWTUtilities.isEmpty(selectAtrasos)){
				select += "\n";
				select += "UNION ALL";
				select += "\n";
				select += selectAtrasos;
				System.out.println(selectAtrasos);
			}			
		}
		...
		if ( listDespesas ){
			select += daoCP.getSelectTitulosAVencerMesAtual();
			select += "UNION ALL";
			select += "\n";
			
			select += daoCP.getSelectOutrosMesesPago();
			if ( optionView.isShowPrevisoes() ){
				select += "\n";
				select += "UNION ALL";
				select += "\n";
			...
			String selectAtrasos = daoCP.getSelectAtrasos();		
			if (  optionView.isShowAtrasos() && ! HowMGWTUtilities.isEmpty(selectAtrasos)){
				select += "\n";
				select += "UNION ALL";
				select += "\n";
