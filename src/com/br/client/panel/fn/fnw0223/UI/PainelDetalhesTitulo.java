...
import com.br.client.panel.fn.fnw0223.model.DataSourceDetalheTitulo;
			...
			HowMGWTCallImpl call = new HowMGWTCallImpl() {
				...
				public void onSuccess(HowMGWTEntity result) {
					...
					if( "Despesa".equals(TipoConta))
						sql = "select hist.dt_referencia, hist.vl_baixa from fn_titulo_pagar_hist as hist where hist.cod_empresa = "+ Configuracao.getCodEmpresa() +" and hist.cod_titulo = "+codTitulo+" and hist.cod_parcela = "+codParcela+" and hist.cod_fornecedor = " +pessoa+ " and hist.cod_especie = '"+chave+ "' and tp_hist = 30 order by  hist.dt_referencia desc";
					else
						sql = "select hist.dt_referencia, hist.vl_baixa from fn_titulo_receber_hist as hist where hist.cod_empresa = "+ Configuracao.getCodEmpresa() +" and hist.cod_titulo = "+codTitulo+" and hist.cod_parcela = "+codParcela+" and  hist.serie = '" +chave+ "' and tp_hist = 30 order by  hist.dt_referencia desc";
