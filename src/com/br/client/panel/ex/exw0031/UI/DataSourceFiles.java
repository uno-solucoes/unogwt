...
public class DataSourceFiles {
	...
	WindowExtranetGED windowExtranetGED;
	...
	public void onExecuteQuery(String text){
			...
			public void onResponse(HowMGWTFormBean formBean) {
				...
				if( bean.getArquivoGEDs()!= null ){
					if( bean.getArquivoGEDs().length == 0 ){
						getWindowExtranetGED().getLabelArquivosEncontrados().setContents("Nenhum arquivo encontrado...");
					}
					if( bean.getArquivoGEDs().length == 1 ){
						getWindowExtranetGED().getLabelArquivosEncontrados().setContents(""+bean.getArquivoGEDs().length+" arquivo encontrado...");
					}
					else{
						getWindowExtranetGED().getLabelArquivosEncontrados().setContents(""+bean.getArquivoGEDs().length+" arquivos encontrados...");
					}				
				}
				else{
					getWindowExtranetGED().getLabelArquivosEncontrados().setContents("Nenhum arquivo encontrado...");
		...
		formBean.setFindFilter(text);
 		struts.request("exw0031.do?method=buscaAvancada",  "EXW0031Form", formBean.toSendBody(""));		
	}
	...
	public WindowExtranetGED getWindowExtranetGED() {
		return windowExtranetGED;
	}


	public void setWindowExtranetGED(WindowExtranetGED windowExtranetGED) {
		this.windowExtranetGED = windowExtranetGED;
	}
}
